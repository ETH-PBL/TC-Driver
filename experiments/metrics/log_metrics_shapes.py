...
with open('./thetas.txt') as file:
    line = file.readline()
    thetas = [float(th) for th in line.split(',')]
    N_RUNS = len(thetas)
...
def main(max_time: int, track_shrink_coeff, track_name: str = 'SOCHI', mode: str = 'MPCC', start_par: int = 0):
    ...
    for exp_idx, th in enumerate(thetas):
        # initialise point
        ...
        data_coords.to_csv('./coords_mpc_{}.csv'.format(track_name))
        ...
        data_runs.to_csv('./runs_data_mpc_{}.csv'.format(track_name))
