...
with open('/home/gnone/edoardo-ghignone/drivers/src/config/MPC_params.yaml') as file:
    MPC_params = yaml.safe_load(file)

with open('/home/gnone/edoardo-ghignone/drivers/src/config/car_params.yaml') as file:
...
def main(max_time: int, track_shrink_coeff, track_name: str = 'SOCHI', mode: str = 'MPCC', start_par: int = 0):
    ...
    for exp_idx, mu in enumerate([1]):
        # initialise point
        start_point = track.get_coordinate(0)
        start_angle = track.get_angle(0)
        start_pose = np.concatenate((start_point, [start_angle]), axis = 0).reshape(1,-1)
        theta = 0

        MPC_params['start_par'] = 0
        
        driver = MPCCDriver(str(track_path) + '_waypoints.txt', track_shrink_coeff, MPC_params)

        env_car_params['mu'] = mu
        env = gym.make('f110_gym:f110direct-v0', map = track_path, map_ext=".png", num_agents=1, params=env_car_params)
        env = Monitor(
            env,
            './videos_mpc/',
            force=True)
