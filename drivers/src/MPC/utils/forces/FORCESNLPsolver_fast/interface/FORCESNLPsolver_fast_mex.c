...
static FORCESNLPsolver_fast_params params;
static FORCESNLPsolver_fast_output output;
static FORCESNLPsolver_fast_info info;
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1)
	{
		mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help FORCESNLPsolver_fast_mex' for details.");
	}    
	if (nlhs > 3) 
	{
    ...
    if( mxGetM(par) != 252 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [252 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,252);
    ...
    if( mxGetM(par) != 315 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [315 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.all_parameters,315);

	}


	#if SET_PRINTLEVEL_FORCESNLPsolver_fast > 0
	...
	/* call solver */
	exitflag = FORCESNLPsolver_fast_solve(&params, &output, &info, fp, pt2function_FORCESNLPsolver_fast);
