...
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
   ...
   OUTPUT = MPCC_solv_20N_no_warm_no_hard_gnone_py.MPCC_solv_20N_no_warm_no_hard_gnone_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = MPCC_solv_20N_no_warm_no_hard_gnone_py.MPCC_solv_20N_no_warm_no_hard_gnone_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = MPCC_solv_20N_no_warm_no_hard_gnone_py.MPCC_solv_20N_no_warm_no_hard_gnone_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'MPCC_solv_20N_no_warm_no_hard_gnone/lib/MPCC_solv_20N_no_warm_no_hard_gnone.so')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../lib/MPCC_solv_20N_no_warm_no_hard_gnone.so'))
    csolver = getattr(_lib,'MPCC_solv_20N_no_warm_no_hard_gnone_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../lib/libMPCC_solv_20N_no_warm_no_hard_gnone.so'))
    csolver = getattr(_lib,'MPCC_solv_20N_no_warm_no_hard_gnone_solve')

class MPCC_solv_20N_no_warm_no_hard_gnone_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
MPCC_solv_20N_no_warm_no_hard_gnone_params = {'c_1' : np.array([]),
'c_2' : np.array([]),
'c_3' : np.array([]),
'c_4' : np.array([]),
'c_5' : np.array([]),
...
MPCC_solv_20N_no_warm_no_hard_gnone_params_types = {'c_1' : np.float64,
'c_2' : np.float64,
'c_3' : np.float64,
'c_4' : np.float64,
'c_5' : np.float64,
...
class MPCC_solv_20N_no_warm_no_hard_gnone_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
MPCC_solv_20N_no_warm_no_hard_gnone_outputs = {'X' : np.array([]),
}


class MPCC_solv_20N_no_warm_no_hard_gnone_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(MPCC_solv_20N_no_warm_no_hard_gnone_params_ctypes), ctypes.POINTER(MPCC_solv_20N_no_warm_no_hard_gnone_outputs_ctypes), ctypes.POINTER(MPCC_solv_20N_no_warm_no_hard_gnone_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def MPCC_solv_20N_no_warm_no_hard_gnone_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v5.1.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = MPCC_solv_20N_no_warm_no_hard_gnone_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=MPCC_solv_20N_no_warm_no_hard_gnone_params_types[par], requirements='F')
    ...
    outputs_py = MPCC_solv_20N_no_warm_no_hard_gnone_outputs_ctypes()
    info_py = MPCC_solv_20N_no_warm_no_hard_gnone_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.MPCC_solv_20N_no_warm_no_hard_gnone_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp)  )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.MPCC_solv_20N_no_warm_no_hard_gnone_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None )
            else:
                exitflag = _lib.MPCC_solv_20N_no_warm_no_hard_gnone_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp )
            libc.fclose(fp)
    ...
    # convert outputs
    for out in MPCC_solv_20N_no_warm_no_hard_gnone_outputs:
        MPCC_solv_20N_no_warm_no_hard_gnone_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return MPCC_solv_20N_no_warm_no_hard_gnone_outputs,int(exitflag),info_py

solve = MPCC_solv_20N_no_warm_no_hard_gnone_solve
